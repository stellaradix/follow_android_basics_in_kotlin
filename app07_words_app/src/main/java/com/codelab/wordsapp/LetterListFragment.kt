...
class LetterListFragment : Fragment() {
	...
	private fun chooseLayout() {
		recyclerView.layoutManager =
			if (isLinearLayoutManager) LinearLayoutManager(this.requireContext())
			else GridLayoutManager(this.requireContext(), 3)
	}
	...
	private fun setIcon(menuItem: MenuItem?) {
		...
		menuItem.icon = ContextCompat.getDrawable(
			requireContext(),
			if (isLinearLayoutManager) R.drawable.ic_grid_layout
