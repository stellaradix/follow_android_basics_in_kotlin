package com.codelab.amphibians
...
import com.codelab.amphibians.ui.AmphibianListFragment
...
class InstrumentationTests : BaseTest() {
	...
	fun recycler_view_content() {
		...
		waitForView(withText("Great Basin Spadefoot"))
			.check(matches(isDisplayed()))
		waitForView(withText("Tiger Salamander"))
			.check(matches(isDisplayed()))
	}
	...
	fun detail_content() {
		...
		waitForView(withText("Blue Jeans Frog"))
			.perform(click())
		waitForView(withText("Blue Jeans Frog"))
			.check(matches(isDisplayed()))
		waitForView(
